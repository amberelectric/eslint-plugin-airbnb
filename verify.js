...
// Verification script for eslint-plugin-airbnb

console.log("🔍 Verifying eslint-plugin-airbnb...\n");
...
try {
  ...
  if (missingConfigs.length > 0) {
    ...
    console.log(
      `✅ All ${expectedConfigs.length} configs available: ${actualConfigs.join(
        ", "
      )}`
    );
  }
  ...
  if (!Array.isArray(reactConfigs)) {
    ...
    console.log(
      `❌ Recommended config should have 3 configs, has ${reactConfigs.length}`
    );
    hasErrors = true;
  } else {
    console.log(
      `✅ Recommended config is array of ${reactConfigs.length} configs`
    );
...
if (hasErrors) {
  ...
  console.log("\n📦 eslint-plugin-airbnb is ready to use!");
  console.log("\nUsage:");
  console.log('  const amber = require("eslint-plugin-airbnb");');
